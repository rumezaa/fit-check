...
<div class="grid">
  {#each app.saved as look (look.id)}
    <div class="card chrome chrome--inset">
      <div class="pair">
        {#each [look.top, look.bottom] as piece}
          <div class="half">
...
<style>
  ...
  .grid { position: absolute; left: 14px; right: 14px; top: 42px; bottom: 44px;
          ...
          grid-auto-rows: 180px; gap: 12px; overflow: auto; align-content: start; }
  ...
  .pair { display: grid; grid-template-rows: 1fr 1fr; gap: 4px; height: 142px; }
  .half { display: grid; place-items: center; overflow: hidden; }
