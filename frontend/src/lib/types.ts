...
export interface TryOnResponse {
  ...
  pairs_with: Category
  /** Every piece on the opposite rail it was held up against, including the
      ones the engine then refused to pair it with. */
  tried: number
  works: number
  standouts: number
  /** Of `tried`, how many it clashes with outright — the engine drops those
      before ranking, so they never appear in `pairings`. */
  clashes: number
